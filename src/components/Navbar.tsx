...
export default function Navbar() {
  ...
  return (
    <motion.header
      className={`fixed top-0 left-0 right-0 z-50 flex justify-center transition-all duration-300 ${scrolled ? 'py-4 backdrop-blur-xl bg-[#0a0a0f]/80 border-b border-white/5 shadow-lg' : 'py-6 bg-transparent'}`}
      initial={{ y: -100 }}
      animate={{ y: 0 }}
    ...
    >
      <div className="w-full max-w-7xl px-6 md:px-12 flex justify-between items-center">
        ...
        <nav className="hidden md:flex items-center gap-8 text-sm font-medium">
          {links.map((link) => (
            <a key={link} href={`#${link.toLowerCase()}`} className="text-slate-300 hover:text-[#00ffcc] transition-colors relative group">
              {link}
      ...
      <AnimatePresence>
        {mobileOpen && (
          ...
          >
            {links.map((link) => (
              <a key={link} href={`#${link.toLowerCase()}`} className="text-xl font-medium text-slate-300 hover:text-[#00ffcc]" onClick={() => setMobileOpen(false)}>
                {link}
              </a>
            ))}
          </motion.div>
